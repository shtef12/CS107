  ...
  while(!partialPaths.empty()){
	...
	for(int j = 0; j < cast.size(); j++){
	  ...
	  if(castIt == previouslySeenActors.end()){
	    ...
	    path pathToActor = actorPath;
	    pathToActor.addConnection(actorFilms[i],cast[j]);
	    //if actor is the target actor return
	    if(cast[j] == target){
	      cout << pathToActor;
	      return;
	    }
