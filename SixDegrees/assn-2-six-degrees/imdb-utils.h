...
inline const char *determinePathToData(const char *userSelectedPath = NULL)
{
  ...
  cout << "get ostype" << endl;
  const char *ostype = getenv("OSTYPE");
  if (strcasecmp(ostype, "linux") == 0)
    return "/usr/Home/Downloads/SixDegrees/assn-2-six-degrees-data/little-endian/";
  if (strcasecmp(ostype, "solaris") == 0)
    return "/usr/Home/Downloads/SixDegrees/assn-2-six-degrees-data/big-endian/";
