...
class imdb {
 ...
 private:
  ...
  vector<string> getActorsFromMovie(void* pActorOffsets, short numOfActors) const;

  int binarySearchFile(int min, int max, string toFind, const void* pFile) const;

  /**
   *Method GetActorName(int offset)
   *===============================
   *Returns a string representation of the actor's name in the actor file at the
   *given offset
   */
  string getActorName(int offset) const;

  /**
   *Method isActorInMovie(const string& player, int movieOffset)
   *============================================================
   *Returns whether the supplied actor is in the movei or not
   *@param the name of the actor to search
   *@param the offset of the movie in the movie file
   *@return true if the name is in the movie, false if not
   */
  bool isActorInMovie(const string& player, int movieOffset, vector<film>& films) const;
